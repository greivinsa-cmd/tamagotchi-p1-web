import { createRuleForPlugin, getSettingsFromContext, unsafeDecodeSettings, DEFAULT_ESLINT_REACT_SETTINGS } from '@eslint-react/shared';
import * as JSX8 from '@eslint-react/jsx';
import * as AST11 from '@eslint-react/ast';
import { isClassComponent, isThisSetState, isChildrenCount, isChildrenForEach, isChildrenMap, isChildrenOnly, isChildrenToArray, useComponentCollectorLegacy, isComponentDidCatch, isGetDerivedStateFromError, isCloneElementCall, isComponentNameLoose, isCreateRefCall, isAssignmentToThisState, isForwardRefCall, useComponentCollector, DEFAULT_COMPONENT_HINT, ERComponentFlag, getInstanceId, isInstanceIdEqual, DISPLAY_NAME_ASSIGNMENT_SELECTOR, isCreateContextCall, isChildrenToArrayCall, ERComponentHint, isDirectValueOfRenderPropertyLoose, isDeclaredInRenderPropLoose, isInsideRenderMethod, isCreateElementCall, isReactHookCall, isReactHookCallWithNameAlias, isGetDerivedStateFromProps, isInitializedFromReact } from '@eslint-react/core';
import { _, getOrUpdate, constTrue, constFalse, identity } from '@eslint-react/eff';
      ...
      JSXAttribute(node) {
        if (node.value === null) {
          context.report({
            ...
            node,
            data: {
              propName: JSX8.getAttributeName(node)
            },
  ...
  create(context) {
    ...
    return {
      ...
      MemberExpression(node) {
        if (!AST11.isThisExpression(node.object)) {
          return;
        }
        ...
        if (AST11.getPropertyName(node.property) !== "state") {
          return;
        }
      ...
      VariableDeclarator(node) {
        ...
        if (node.init == null || !AST11.isThisExpression(node.init) || node.id.type !== AST_NODE_TYPES.ObjectPattern) {
          return;
        }
        const hasState = node.id.properties.some(
          (prop) => prop.type === AST_NODE_TYPES.Property && isKeyLiteral(prop, prop.key) && AST11.getPropertyName(prop.key) === "state"
        );
        if (!hasState) {
          return;
        }
...
var no_children_prop_default = createRule({
  ...
  create(context) {
    return {
      JSXElement(node) {
        const attribute = JSX8.getAttribute(
          "children",
...
var no_comment_textnodes_default = createRule({
  ...
  create(context) {
    function hasCommentLike(node) {
      if (AST11.isOneOf([AST_NODE_TYPES.JSXAttribute, AST_NODE_TYPES.JSXExpressionContainer])(node.parent)) {
        return false;
      }
    ...
    const visitorFunction = (node) => {
      if (!AST11.isOneOf([AST_NODE_TYPES.JSXElement, AST_NODE_TYPES.JSXFragment])(node.parent)) {
        return;
      }
...
var no_complex_conditional_rendering_default = createRule({
  ...
  create(context) {
    const visitorFunction = (node) => {
      ...
      if (!AST11.is(AST_NODE_TYPES.JSXExpressionContainer)(jsxExpContainer)) {
        return;
      }
...
function isComponentWillMount(node) {
  return AST11.isMethodOrProperty(node) && node.key.type === AST_NODE_TYPES.Identifier && node.key.name === "componentWillMount";
}
...
function isComponentWillUpdate(node) {
  return AST11.isMethodOrProperty(node) && node.key.type === AST_NODE_TYPES.Identifier && node.key.name === "componentWillReceiveProps";
}
...
function isComponentWillUpdate2(node) {
  return AST11.isMethodOrProperty(node) && node.key.type === AST_NODE_TYPES.Identifier && node.key.name === "componentWillUpdate";
}
...
var no_context_provider_default = createRule({
  ...
  create(context) {
    ...
    return {
      JSXElement(node) {
        const fullName = JSX8.getElementType(node);
...
var no_create_ref_default = createRule({
  ...
  create(context) {
    return {
      CallExpression(node) {
        if (isCreateRefCall(context, node) && AST11.findParentNode(node, isClassComponent) == null) {
  ...
  create(context) {
    ...
    return {
      AssignmentExpression(node) {
        ...
        if (!AST11.isFunction(variableNode) && !isClassComponent(variableNode)) return;
...
function isConstructorFunction(node) {
  return AST11.isOneOf([AST_NODE_TYPES.FunctionDeclaration, AST_NODE_TYPES.FunctionExpression])(node) && AST11.isMethodOrProperty(node.parent) && node.parent.key.type === AST_NODE_TYPES.Identifier && node.parent.key.name === "constructor";
}
  ...
  create(context) {
    return {
      AssignmentExpression(node) {
        ...
        const parentClass = AST11.findParentNode(
          node,
          AST11.isOneOf([
        ...
        if (isClassComponent(parentClass) && context.sourceCode.getScope(node).block !== AST11.findParentNode(node, isConstructorFunction)) {
          context.report({
    ...
    return {
      "JSXAttribute[name.name='key']"(node) {
        ...
        switch (jsxElement.parent.type) {
          ...
          default: {
            const call = AST11.findParentNode(jsxElement, AST11.isMapCallLoose);
            const iter = AST11.findParentNode(jsxElement, (n) => n === call || AST11.isFunction(n));
            if (!AST11.isFunction(iter)) return;
            const arg0 = call?.arguments[0];
            if (call == null || arg0 == null) return;
            if (AST11.getEcmaExpression(arg0) !== iter) {
              return;
            }
...
function getFix(context, node) {
  return (fixer) => {
    ...
    if (componentNode == null || !AST11.isFunction(componentNode)) {
      return [];
    }
    return [
...
function getComponentPropsFixes(context, fixer, node, typeArguments) {
  ...
  const fixedArg0Text = match(arg0).with({ type: AST_NODE_TYPES.Identifier }, (n) => `...${n.name}`).with({ type: AST_NODE_TYPES.ObjectPattern }, (n) => n.properties.map(getText).join(", ")).otherwise(() => _);
  const fixedArg1Text = match(arg1).with(P.nullish, () => "ref").with({ type: AST_NODE_TYPES.Identifier, name: "ref" }, () => "ref").with({ type: AST_NODE_TYPES.Identifier, name: P.string }, (n) => `ref: ${n.name}`).otherwise(() => _);
...
var no_implicit_key_default = createRule({
  ...
  create(context) {
    return {
      JSXOpeningElement(node) {
        const initialScope = context.sourceCode.getScope(node);
        const keyPropFound = JSX8.getAttribute("key", node.attributes, initialScope);
...
var no_leaked_conditional_rendering_default = createRule({
  ...
  create(context) {
    ...
    function getReportDescriptor(node) {
      ...
      return match(node).when(AST11.isJSX, () => _).with({ type: AST_NODE_TYPES.LogicalExpression, operator: "&&" }, ({ left, right }) => {
        const isLeftUnaryNot = left.type === AST_NODE_TYPES.UnaryExpression && left.operator === "!";
        if (isLeftUnaryNot) {
          return getReportDescriptor(right);
        }
        const initialScope = context.sourceCode.getScope(left);
        const isLeftNan = left.type === AST_NODE_TYPES.Identifier && left.name === "NaN" || VAR.toStaticValue({ kind: "lazy", node: left, initialScope }).value === "NaN";
        if (isLeftNan) {
          return {
        ...
        const initExpression = match(variable?.defs.at(0)?.node).with({ init: P.select({ type: P.not(AST_NODE_TYPES.VariableDeclaration) }) }, identity).otherwise(() => _);
    ...
    return {
      ...
      "Program:exit"(node) {
        ...
        for (const { node: node2, displayName, flag } of components.values()) {
          ...
          if (AST11.getFunctionIdentifier(node2) != null) {
            continue;
          }
  ...
  create(context) {
    ...
    function checkIteratorElement(node) {
      switch (node.type) {
        case AST_NODE_TYPES.JSXElement: {
          const initialScope = context.sourceCode.getScope(node);
          if (!JSX8.hasAttribute("key", node.openingElement.attributes, initialScope)) {
            return {
    ...
    function checkBlockStatement(node) {
      const descriptors = [];
      for (const statement of AST11.getNestedReturnStatements(node)) {
    ...
    return {
      ArrayExpression(node) {
        ...
        for (const element of elements) {
          if (!JSX8.hasAttribute("key", element.openingElement.attributes, initialScope)) {
            context.report({
              messageId: "missingKey",
              node: element
            });
      ...
      CallExpression(node) {
        ...
        const isMapCallLike = AST11.isMapCallLoose(node);
...
function isArrayFromCall(node) {
  return node.type === AST_NODE_TYPES.CallExpression && node.callee.type === AST_NODE_TYPES.MemberExpression && node.callee.property.type === AST_NODE_TYPES.Identifier && node.callee.property.name === "from";
}
...
var no_nested_components_default = createRule({
  ...
  name: RULE_NAME30,
  create(context) {
    const hint = ERComponentHint.SkipMapCallback | ERComponentHint.SkipNullLiteral | ERComponentHint.SkipUndefined | ERComponentHint.SkipBooleanLiteral | ERComponentHint.SkipStringLiteral | ERComponentHint.SkipNumberLiteral | ERComponentHint.StrictLogical | ERComponentHint.StrictConditional;
    ...
    return {
      ...
      "Program:exit"(node) {
        ...
        for (const { name: name5, node: component } of functionComponents) {
          ...
          const isInsideJSXPropValue = component.parent.type === AST_NODE_TYPES.JSXAttribute || JSX8.findParentAttribute(node, (n) => n.value?.type === AST_NODE_TYPES.JSXExpressionContainer) != null;
          ...
          const parentComponent = AST11.findParentNode(component, isFunctionComponent);
          const isParentComponentNotDirectValueOfRenderProperty = parentComponent != null && !isDirectValueOfRenderPropertyLoose(parentComponent);
        ...
        for (const { name: name5 = "unknown", node: component } of classComponents) {
          if (AST11.findParentNode(component, (n) => isClassComponent11(n) || isFunctionComponent(n)) == null) {
            continue;
          }
          context.report({
            messageId: "nestedComponent",
...
function isInsideCreateElementProps(context, node) {
  const call = AST11.findParentNode(node, isCreateElementCall(context));
  if (call == null) return false;
  const prop = AST11.findParentNode(node, AST11.is(AST_NODE_TYPES.ObjectExpression));
  ...
  create(context) {
    ...
    return {
      AssignmentExpression(node) {
        ...
        const variable = VAR.findVariable(object.name, context.sourceCode.getScope(node));
        const variableNode = VAR.getVariableInitNode(variable, 0);
        if (variableNode != null && (AST11.isFunction(variableNode) || isClassComponent(variableNode))) {
...
function isShouldComponentUpdate(node) {
  return AST11.isMethodOrProperty(node) && node.key.type === AST_NODE_TYPES.Identifier && node.key.name === "shouldComponentUpdate";
}
...
function isComponentDidMount(node) {
  return AST11.isMethodOrProperty(node) && node.key.type === AST_NODE_TYPES.Identifier && node.key.name === "componentDidMount";
}
  ...
  create(context) {
    ...
    return {
      CallExpression(node) {
        ...
        const clazz = AST11.findParentNode(node, isClassComponent);
        const method = AST11.findParentNode(node, (n) => n === clazz || isComponentDidMount(n));
...
function isComponentDidUpdate(node) {
  return AST11.isMethodOrProperty(node) && node.key.type === AST_NODE_TYPES.Identifier && node.key.name === "componentDidUpdate";
}
  ...
  create(context) {
    ...
    return {
      CallExpression(node) {
        ...
        const clazz = AST11.findParentNode(node, isClassComponent);
        const method = AST11.findParentNode(node, (n) => n === clazz || isComponentDidUpdate(n));
        if (clazz == null || method == null || method === clazz) return;
        const methodScope = context.sourceCode.getScope(method);
        const upperScope = context.sourceCode.getScope(node).upper;
        if (method.parent === clazz.body && upperScope === methodScope) {
  ...
  create(context) {
    ...
    return {
      CallExpression(node) {
        ...
        const clazz = AST11.findParentNode(node, isClassComponent);
        const method = AST11.findParentNode(node, (n) => n === clazz || isComponentWillUpdate3(n));
        if (clazz == null || method == null || method === clazz) return;
        const methodScope = context.sourceCode.getScope(method);
        const upperScope = context.sourceCode.getScope(node).upper;
        if (method.parent === clazz.body && upperScope === methodScope) {
...
function containsStringExpressionContainer({ value }) {
  ...
  if (value.expression.type === AST_NODE_TYPES.Literal) {
...
function isUnsafeComponentWillMount(node) {
  return AST11.isMethodOrProperty(node) && node.key.type === AST_NODE_TYPES.Identifier && node.key.name === "UNSAFE_componentWillMount";
}
...
function isUnsafeComponentWillReceiveProps(node) {
  return AST11.isMethodOrProperty(node) && node.key.type === AST_NODE_TYPES.Identifier && node.key.name === "UNSAFE_componentWillReceiveProps";
}
...
function isUnsafeComponentWillUpdate(node) {
  return AST11.isMethodOrProperty(node) && node.key.type === AST_NODE_TYPES.Identifier && node.key.name === "UNSAFE_componentWillUpdate";
}
...
var no_unstable_context_value_default = createRule({
  ...
  create(context) {
    ...
    return {
      ...listeners,
      JSXOpeningElement(node) {
        const fullName = JSX8.getElementType(node.parent);
...
var no_unused_class_component_members_default = createRule({
  ...
  create(context) {
    ...
    function classExit() {
      ...
      const className = AST11.getClassIdentifier(currentClass)?.name;
      ...
      for (const def of defs) {
        const methodName = AST11.getPropertyName(def);
        if (methodName == null) {
          continue;
        }
    ...
    return {
      ...
      MemberExpression(node) {
        ...
        if (!AST11.isThisExpression(node.object) || !isKeyLiteral2(node, node.property)) {
          return;
        }
        ...
        const propertyName = AST11.getPropertyName(node.property);
        ...
        if (node.init != null && AST11.isThisExpression(node.init) && node.id.type === AST_NODE_TYPES.ObjectPattern) {
          for (const prop of node.id.properties) {
            if (prop.type === AST_NODE_TYPES.Property && isKeyLiteral2(prop, prop.key)) {
              const keyName = AST11.getPropertyName(prop.key);
              if (keyName != null) {
...
var no_unused_state_default = createRule({
  ...
  create(context) {
    ...
    function methodEnter(node) {
      ...
      if (AST11.getPropertyName(node.key) === "state") {
      ...
      VariableDeclarator(node) {
        ...
        const hasState = node.id.properties.some((prop) => {
          if (prop.type === AST_NODE_TYPES.Property && isKeyLiteral3(prop, prop.key)) {
            return AST11.getPropertyName(prop.key) === "state";
          }
          return false;
        });
        if (!hasState) {
  ...
  create(context) {
    return {
      CallExpression(node) {
        ...
        if (component == null || !AST11.isFunction(component)) {
          return;
        }
...
function checkAndReport(context, node, allowExpressions) {
  function fix(fixer) {
    if (node.parent.type !== AST_NODE_TYPES.JSXElement && node.parent.type !== AST_NODE_TYPES.JSXFragment) {
      ...
      if (node.children.some(
        (child) => JSX8.isLiteral(child) && !JSX8.isWhiteSpace(child) || AST11.is(AST_NODE_TYPES.JSXExpressionContainer)(child)
      )) {
        return null;
      }
    }
    if (JSX8.isUserDefinedElement(node.parent)) {
      return null;
    }
  ...
  const initialScope = context.sourceCode.getScope(node);
  if (JSX8.isKeyedElement(node, initialScope)) {
    return;
  }
  if (JSX8.isBuiltInElement(node.parent)) {
    context.report({
  ...
  switch (true) {
    ...
    case (allowExpressions && !isChildElement && node.children.length === 1 && JSX8.isLiteral(node.children.at(0))): {
      return;
    }
    // <Foo><>hello, world</></Foo>
...
var no_useless_fragment_default = createRule({
  ...
  create(context, [option]) {
    ...
    return {
      JSXElement(node) {
        if (!JSX8.isFragmentElement(node)) return;
    ...
    return {
      ...
      "Program:exit"(node) {
        ...
        function isFunctionComponent(block) {
          if (!AST11.isFunction(block)) {
            return false;
          }
          const id = AST11.getFunctionIdentifier(block);
        ...
        for (const [initialScope, memberExpression] of memberExpressionWithNames) {
          ...
          const isMatch = (node2) => node2 != null && node2.type === AST_NODE_TYPES.Identifier && node2.name === memberExpression.object.name;
          if (isMatch(props)) {
            context.report({
...
var prefer_shorthand_boolean_default = createRule({
  ...
  create(context) {
    return {
      JSXAttribute(node) {
        const { value } = node;
        const propName = JSX8.getAttributeName(node);
