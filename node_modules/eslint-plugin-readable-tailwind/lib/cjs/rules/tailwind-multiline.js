...
function lintLiterals(ctx, literals) {
    ...
    for (const literal of literals) {
        ...
        if ((0, readable_tailwind_parsers_es_js_1.isESObjectKey)(literal.node)) {
            continue;
        }
        const lineStartPosition = literal.type === "TemplateLiteral"
            ? (0, readable_tailwind_utils_utils_js_1.findLineStartPosition)(ctx, literal.parent) + getIndentation(ctx, indent)
            : (0, readable_tailwind_utils_utils_js_1.findLineStartPosition)(ctx, literal) + getIndentation(ctx, indent);
        const literalStartPosition = (0, readable_tailwind_utils_utils_js_1.findLiteralStartPosition)(ctx, literal);
        ...
        if (literal.openingQuote) {
            if (literal.parent.type === "JSXAttribute" ||
...
class Line {
    ...
    toString(indent = true) {
        return this.join([
            ...
            (0, readable_tailwind_utils_quotes_js_1.escapeNestedQuotes)(this.join(this.classes), this.meta.openingQuote ?? "`"),
