...
function lintLiterals(ctx, literals) {
    for (const literal of literals) {
        ...
        const escapedClasses = (0, readable_tailwind_utils_quotes_js_1.escapeNestedQuotes)(finalChunks.join(""), literal.openingQuote ?? "\"");
        const fixedClasses = [
...
function findParentLiteralNodes(node) {
    if (!(0, readable_tailwind_parsers_es_js_1.hasESNodeParentExtension)(node)) {
        return;
    }
    const parentLiterals = [];
    let currentNode = node;
    while ((0, readable_tailwind_parsers_es_js_1.hasESNodeParentExtension)(currentNode)) {
        const parent = currentNode.parent;
        if ((0, readable_tailwind_parsers_es_js_1.isESCallExpression)(parent)) {
            break;
        }
        if ((0, readable_tailwind_parsers_es_js_1.isESVariableDeclarator)(parent)) {
            break;
        }
...
function getLiteralsFromParentLiteralNodes(parentLiteralNodes, literals) {
