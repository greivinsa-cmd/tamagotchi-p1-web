import * as AST6 from '@eslint-react/ast';
import { flip, dual, constFalse, identity, _ } from '@eslint-react/eff';
import * as JSX3 from '@eslint-react/jsx';
import { DEFAULT_ESLINT_REACT_SETTINGS as DEFAULT_ESLINT_REACT_SETTINGS$1, unsafeDecodeSettings, getId } from '@eslint-react/shared';
...
function getRequireExpressionArguments(node) {
  return match(node).with({ type: AST_NODE_TYPES.CallExpression, arguments: P.select(), callee: { type: AST_NODE_TYPES.Identifier, name: "require" } }, identity).with({ type: AST_NODE_TYPES.MemberExpression, object: P.select() }, getRequireExpressionArguments).otherwise(() => _);
}
function isInitializedFromReact(name, importSource, initialScope) {
  ...
  if (node.type === AST_NODE_TYPES.VariableDeclarator && node.init != null) {
    ...
    const arg0 = args?.[0];
    if (arg0 == null || !AST6.isKindOfLiteral(arg0, "string")) {
      return false;
    }
...
var defaultImportSource = DEFAULT_ESLINT_REACT_SETTINGS$1.importSource;
...
function isFromReactStrict(node, name, importSource, initialScope) {
  switch (node?.type) {
    ...
    case AST_NODE_TYPES.MemberExpression:
      return node.object.type === AST_NODE_TYPES.Identifier && node.property.type === AST_NODE_TYPES.Identifier && node.property.name === name && isInitializedFromReact(node.object.name, importSource, initialScope);
    default:
      return false;
  }
}
function isFromReact(name) {
  return dual(2, (context, node) => {
...
function isInstanceIdEqual(context, a, b) {
  return AST6.isNodeEqual(a, b) || VAR.isNodeValueEqual(a, b, [
...
function isReactHook(node) {
  ...
  const id = AST6.getFunctionIdentifier(node);
...
function getFunctionComponentIdentifier(context, node) {
  const functionId = AST6.getFunctionIdentifier(node);
  if (functionId != null) {
    return functionId;
  }
  const { parent } = node;
  if (parent.type === AST_NODE_TYPES.CallExpression && isComponentWrapperCall(context, parent) && parent.parent.type === AST_NODE_TYPES.VariableDeclarator && parent.parent.id.type === AST_NODE_TYPES.Identifier) {
    return parent.parent.id;
  }
  if (parent.type === AST_NODE_TYPES.CallExpression && isComponentWrapperCall(context, parent) && parent.parent.type === AST_NODE_TYPES.CallExpression && isComponentWrapperCall(context, parent.parent) && parent.parent.parent.type === AST_NODE_TYPES.VariableDeclarator && parent.parent.parent.id.type === AST_NODE_TYPES.Identifier) {
...
function isComponentDidMount(node) {
  return AST6.isMethodOrProperty(node) && node.key.type === AST_NODE_TYPES.Identifier && node.key.name === "componentDidMount";
}
function isComponentWillUnmount(node) {
  return AST6.isMethodOrProperty(node) && node.key.type === AST_NODE_TYPES.Identifier && node.key.name === "componentWillUnmount";
}
function isComponentDidCatch(node) {
  return AST6.isMethodOrProperty(node) && !node.static && node.key.type === AST_NODE_TYPES.Identifier && node.key.name === "componentDidCatch";
}
function isGetDerivedStateFromError(node) {
  return AST6.isMethodOrProperty(node) && node.static && node.key.type === AST_NODE_TYPES.Identifier && node.key.name === "getDerivedStateFromError";
}
function isGetDerivedStateFromProps(node) {
  return AST6.isMethodOrProperty(node) && node.static && node.key.type === AST_NODE_TYPES.Identifier && node.key.name === "getDerivedStateFromProps";
}
function isFunctionOfComponentDidMount(node) {
  return AST6.isFunction(node) && isComponentDidMount(node.parent) && node.parent.value === node;
}
function isFunctionOfComponentWillUnmount(node) {
  return AST6.isFunction(node) && isComponentWillUnmount(node.parent) && node.parent.value === node;
}
function isRenderMethodLike(node) {
  return AST6.isMethodOrProperty(node) && node.key.type === AST_NODE_TYPES.Identifier && node.key.name === "render" && node.parent.parent.type === AST_NODE_TYPES.ClassDeclaration;
}
...
function isInsideRenderMethod(node) {
  return AST6.findParentNode(node, (node2) => isRenderMethodLike(node2) && isClassComponent(node2.parent.parent)) != null;
}
...
function hasValidHierarchy(context, node, hint) {
  ...
  if (hint & ERComponentHint.SkipMapCallback && AST6.isMapCallLoose(node.parent)) {
    return false;
  }
  ...
  const boundaryNode = AST6.findParentNode(
    node,
    AST6.isOneOf([
  ...
  return boundaryNode == null || boundaryNode.type !== AST_NODE_TYPES.JSXExpressionContainer;
}
...
function isFunctionOfClassMethod(node) {
  return (node.type === AST_NODE_TYPES.FunctionExpression || node.type === AST_NODE_TYPES.ArrowFunctionExpression) && node.parent.type === AST_NODE_TYPES.MethodDefinition;
}
function isFunctionOfClassProperty(node) {
  return (node.type === AST_NODE_TYPES.FunctionExpression || node.type === AST_NODE_TYPES.ArrowFunctionExpression) && node.parent.type === AST_NODE_TYPES.Property;
}
function isFunctionOfObjectMethod(node) {
  return (node.type === AST_NODE_TYPES.FunctionExpression || node.type === AST_NODE_TYPES.ArrowFunctionExpression) && node.parent.type === AST_NODE_TYPES.Property && node.parent.parent.type === AST_NODE_TYPES.ObjectExpression;
}
...
function useComponentCollector(context, options = {}) {
  ...
  const onFunctionExit = () => {
    ...
    const shouldDrop = AST6.getNestedReturnStatements(entry.node.body).slice().reverse().some((r) => {
      return context.sourceCode.getScope(r).block === entry.node && r.argument != null && !JSX3.isJSXValue(r.argument, jsxCtx, hint);
    });
    if (shouldDrop) {
  ...
  const listeners = {
    ...
    "ArrowFunctionExpression[type][body.type!='BlockStatement']"() {
      ...
      const isComponent = hasNoneOrLooseComponentName(context, entry.node) && JSX3.isJSXValue(body, jsxCtx, hint) && hasValidHierarchy(context, entry.node, hint);
      if (!isComponent) return;
      const initPath = AST6.getFunctionInitPath(entry.node);
    ...
    "ReturnStatement[type]"(node) {
      const entry = getCurrentEntry();
      if (entry == null) return;
      const isComponent = hasNoneOrLooseComponentName(context, entry.node) && JSX3.isJSXValue(node.argument, jsxCtx, hint) && hasValidHierarchy(context, entry.node, hint);
      if (!isComponent) return;
...
function getComponentFlag(initPath) {
  ...
  if (initPath != null && AST6.hasCallInFunctionInitPath("memo", initPath)) {
    flag |= ERComponentFlag.Memo;
  }
  if (initPath != null && AST6.hasCallInFunctionInitPath("forwardRef", initPath)) {
...
function useComponentCollectorLegacy() {
  ...
  const collect = (node) => {
    ...
    const id = AST6.getClassIdentifier(node);
    const key = getId();
...
var isInversePhase = dual(2, (a, b) => ERPhaseRelevance.get(a) === b);
function isRenderFunctionLoose(context, node) {
  const { body, parent } = node;
  if (AST6.getFunctionIdentifier(node)?.name.startsWith("render")) {
  ...
  return JSX3.isJSXValue(
    ...
    JSX3.JSXValueHint.SkipNullLiteral | JSX3.JSXValueHint.SkipUndefined | JSX3.JSXValueHint.StrictLogical | JSX3.JSXValueHint.StrictConditional
  );
}
function isRenderPropLoose(context, node) {
  ...
  return node.name.name.startsWith("render") && node.value?.type === AST_NODE_TYPES.JSXExpressionContainer && AST6.isFunction(node.value.expression) && isRenderFunctionLoose(context, node.value.expression);
}
...
function isDeclaredInRenderPropLoose(node) {
  ...
  const parent = AST6.findParentNode(node, AST6.is(AST_NODE_TYPES.JSXExpressionContainer))?.parent;
...
function isThisSetState(node) {
  const { callee } = node;
  return callee.type === AST_NODE_TYPES.MemberExpression && AST6.isThisExpression(callee.object) && callee.property.type === AST_NODE_TYPES.Identifier && callee.property.name === "setState";
}
function isAssignmentToThisState(node) {
  const { left } = node;
  return left.type === AST_NODE_TYPES.MemberExpression && AST6.isThisExpression(left.object) && AST6.getPropertyName(left.property) === "state";
}
